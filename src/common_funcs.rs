...
pub fn link_program(
    ...
    let program = gl.create_program().ok_or_else(|| String::from("Could not create program"))?;
        ...
        .unwrap_or(false)
    {
        ...
        Err(gl.get_program_info_log(&program)
            .unwrap_or_else(|| String::from("Could not create program object")))
    }
}


fn compile_shader(gl: &WebGlRenderingContext, shader_type: u32, source: &str) -> Result<WebGlShader, String> {
    let shader = gl.create_shader(shader_type).ok_or_else(|| String::from("Could not create shader"))?;
    ...
    if gl.get_shader_parameter(&shader, WebGlRenderingContext::COMPILE_STATUS).as_bool().unwrap_or(false) {
        ...
        Err(gl.get_shader_info_log(&shader).unwrap_or_else(|| String::from("Unable to et shader info log")))
    }
}
...
pub fn mult_matrix_4(a: [f32; 16], b: [f32; 16]) -> [f32; 16] { // TODO improve
